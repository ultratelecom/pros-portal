-- Safe database setup script that handles existing objects
-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Drop existing triggers if they exist
DROP TRIGGER IF EXISTS update_employee_updated_at ON "Employee";
DROP TRIGGER IF EXISTS update_supervisor_updated_at ON "Supervisor";

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Enable all access for Employee" ON "Employee";
DROP POLICY IF EXISTS "Enable all access for Supervisor" ON "Supervisor";
DROP POLICY IF EXISTS "Enable all access for CheckLog" ON "CheckLog";
DROP POLICY IF EXISTS "Enable all access for AdminCode" ON "AdminCode";
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'CheckLog_employeeId_fkey'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'CheckLog_verifiedById_fkey'
    ) THEN
...
-- Create triggers (after dropping them above)
...
-- Create new policies (after dropping them above)
